...
KEYWORD="${KEYWORD:-132.231.8.189\(}"
HIGH_RR_PRIO="${HIGH_RR_PRIO:-90}"
LOW_RR_PRIO="${LOW_RR_PRIO:-90}"
...
set_clients_rr() {
  echo "Setting SCHED_RR prio $HIGH_RR_PRIO and pinning to CPUs $CPULIST for processes containing '$KEYWORD'..."

  mapfile -t high_pids < <(pgrep -f "$KEYWORD" || true)
  client_count=${#high_pids[@]}

  if (( client_count == 0 )); then
    echo "Error: could not find foreground client/backend PIDs matching KEYWORD='$KEYWORD'"
    exit 1
  fi

  for pid in "${high_pids[@]}"; do
    echo "  -> PID $pid (client, SCHED_RR)"
    sudo chrt -r -p "$HIGH_RR_PRIO" "$pid"
    sudo taskset -pc "$CPULIST" "$pid" >/dev/null
  done
}

set_clients_nice() {
  echo "Setting client backends to nice -20 and pinning to CPUs $CPULIST for processes containing '$KEYWORD'..."
...
set_madlibs_rr() {
  echo "Setting MADlib backends to SCHED_RR prio $LOW_RR_PRIO and pinning to CPUs $CPULIST..."

  if [[ "${#madlib_pids[@]}" -eq 0 ]]; then
    echo "Error: no MADlib backend PIDs available for SCHED_RR assignment"
    exit 1
  fi

  for pid in "${madlib_pids[@]}"; do
    echo "  -> PID $pid (MADlib, SCHED_RR)"
    sudo chrt -r -p "$LOW_RR_PRIO" "$pid"
    sudo taskset -cp "$CPULIST" "$pid" >/dev/null
  done
}
...
if [[ "$no_clients" == true ]]; then
  client_count=0
  echo "Skipping client SCHED_RR block (--no-clients)."
