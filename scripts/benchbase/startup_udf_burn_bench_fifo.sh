...
if [[ "$no_clients" == true ]]; then
  ...
  if (( client_count )); then
    for pid in "${high_pids[@]}"; do
      echo "  -> PID $pid (HW)"
      sudo chrt -f -p "$HIGH_FIFO_PRIO" "$pid"
      echo "$CPULIST"
...
if [[ "$same_prio" != true ]]; then
  ...
  if [[ "$read_only" == true ]]; then
    ...
    if [[ "$same_prio" == true ]]; then
      outfile="results/udf_${client_count}_${udf_count}_ff_same_prio.csv"
    else
      outfile="results/udf_${client_count}_${udf_count}_ff.csv"
    fi
...
if (( udf_count )); then
  for pid in "${low_pids[@]}"; do
    echo "  -> PID $pid (LW)"
    sudo chrt -f -p "$LOW_FIFO_PRIO" "$pid"
    echo "$CPULIST"
    sudo taskset -pc "$CPULIST" "$pid" >/dev/null
  done
else
