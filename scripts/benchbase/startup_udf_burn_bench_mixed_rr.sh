...
ensure_cgroup() {
  ...
  # Keep cpuset handling disabled here, as in the original script.
...
set_clients_rr() {
  echo "Setting client backends to SCHED_RR prio $HIGH_RR_PRIO and pinning to CPUs $CPULIST..."
...
if [[ "$run_udf" == true ]]; then
  if [[ "$idle" == true ]]; then
    ...
    if [[ "$same_prio" == true ]]; then
      echo "Warning: --same-prio was passed, but UDFs are still assigned to lw/lw_high cgroups and are not switched to SCHED_RR."
    fi
